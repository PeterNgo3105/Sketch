...
void define_Author() {
    // Author
    std::cout << "*************************************************\n";
    std::cout << " ELEX4618 Grade System, by Peter Ngo\n";
    std::cout << "*************************************************\n\n";
}
void print_Menu() {

    std::cout << "Menu:" << std::endl;		//print out Menu 
    std::cout << "(1) Toggling Blue LED\n"
        << "(2) Digital Test\n"
        << "(3) Button Test\n"
        << "(4) Analog Joystick\n"
        << "(5) Servo Rotate\n";
            ...
            if(input == 1) {
                ...
                while (running) {
                    if (_kbhit()) {           // check if key pressed (non-blocking)
                        ...
                        val = 1;
                        int gval;
                        bool r_set = Ccontrol.set_data(type, channel, val);
                        if (r_set && Ccontrol.get_data(type, channel, gval)) {
                            Ccontrol.set_data(type, channel, val);
                            Ccontrol.get_data(type, channel, gval);
                            std::cout << "BLUE LED STATE:  " << gval << "\n";
                        }
                        else {
                ...
                while (running) {
                    if (_kbhit()) {           // check if key pressed (non-blocking)
                        ...
                        type = DIGITAL;
                        channel = 32;
                        if (Ccontrol.get_data(type, channel, val))
                        {
                            Ccontrol.get_data(type, channel, val);
                            if (!val) {
                                value++;
                                std::cout << "DIGITAL TEST: CH " << channel << "= " << value << "\n";
                            }
                ...
                while (running){
                    if (_kbhit()) {           // check if key pressed (non-blocking)
                        ...
                        {
                            Ccontrol.get_data(type, channel, val);
                            if (Ccontrol.get_button(val)) {
                                value++;
                                std::cout << "BUTTON TEST: CH " << channel << "= " << value << "\n";
                            }
                ...
                Ccontrol.init_com(port);
                float x_percent_value = 0;
                float y_percent_value = 0;
                while (running) {
                    if (_kbhit()) {           // check if key pressed (non-blocking)
                        char ch = _getch();   // read key
                        if (ch == 27) {       // ESC key ASCII = 27
                        ...
                        {
                            Ccontrol.get_data(type, channel, val);
                            Ccontrol.get_data(type, y_channel, y_val);
                            x_percent_value = static_cast<float>(val);
                            y_percent_value = static_cast<float>(y_val);
                            std::cout << std:: left << "ANALOG TEST: CH " <<std::setw(2) << channel 
                                << std:: setw(2) << "="<< std::setw(6) << val << std::setw(5) << std::setprecision(3) << Ccontrol.get_analog(x_percent_value) << "%"<< std::setw(5) <<" ";
                            std::cout << std::setw(3)<< "CH " << std::setw(2) << y_channel 
                                << std::setw(2) << "=" << std::setw(6) << y_val << std::setw(5) << std::setprecision(3) << Ccontrol.get_analog(y_percent_value) << "%\n";
                        }
                        else {
                            std::cout << " program failed\n";
                            break;
              ...
              while (running) {
                  if (_kbhit()) {           // check if key pressed (non-blocking)
                      ...
                      channel = 0;
                      int g_val;
                      if (Ccontrol.set_data(type, channel, val))
                      {
                          //val = 1;
                          for (int angle = 1; angle <= 180; angle = angle + 2) {
                              if (running) {
                                  ...
                                  {
                                      Ccontrol.set_data(type, channel, angle);
                                      Ccontrol.get_data(type, channel, g_val);
                                      std::cout << "SERVO TEST: CH " << channel << "  " << g_val << "\n";
                                  }
                              }
                              else
                              {
                              ...
                              //Sleep(500);
                              for (int angle = 180; angle >= 1; angle = angle - 2) {
                                  if (running){
